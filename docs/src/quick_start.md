...
Belinda uses the following variable name conventions
...
The latter can be generated by running the following command (`runleiden` from https://pypi.org/project/runleiden/):
...
Now let's actually load the clustering. Leiden clusters are given in the so-called "membership" format (see [here](./clustering_formats.md)).
...
First of all, Belinda operates under the data model that
...
With that explained, let's take a closer look at this clustering (i.e., set of clusters). First, it makes sense to reorder the clusters by sorting the biggest first. We just reuse what is provided in Polars for us to sort this data frame:
